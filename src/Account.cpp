...
//Retrieve admin managable accounts \/\/\/
...
//Admin Account Managment Functions (for individual accounts) \/\/\/\/
...
//Admin: reset an account's login attempts
...
//Admin: Reset an account's password
...
//set a managable account's type
...
std::vector<std::string> Account::RetrieveAccountMessages(ReferenceValidationMechanism *r){
    ...
    try
    {
        ...
        pstmt->setString(1, r->GetAccount().userName);
        sql::ResultSet *res = pstmt->executeQuery();
...
//send message to all admins of this network
